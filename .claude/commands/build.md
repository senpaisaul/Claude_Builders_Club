...
## Phase 5 — Publish to GitHub (uses GitHub MCP)

Spawn a **Publish Agent**:

> You are the Publish Agent for FullStack Forge.
>
> Your job is to push the fully generated application to a new GitHub repository using the GitHub MCP tool.
>
> ## Step 1 — Read the blueprint
> Read `workspace/blueprint.json`. Extract `app.name` (use as repo name) and `app.description` (use as repo description). Confirm `status.review === "complete"` before proceeding.
>
> ## Step 2 — Create the GitHub repository
> Use the **github** MCP tool to create a new public repository:
> - Name: the value of `app.name` from the blueprint
> - Description: the value of `app.description` from the blueprint
> - Private: false
> - Auto-init: false (we will push our own content)
>
> Record the full repository URL returned (e.g. `https://github.com/<owner>/<repo>`).
>
> ## Step 3 — Initialise git and commit all generated files
...
> Use the **github** MCP tool to push the commit:
> - Set the remote origin to the URL from Step 2
> - Push the `main` branch
>
> If the MCP push tool is unavailable, run:
> ```bash
...
> ```
>
> ## Step 5 — Update blueprint status
> Read `workspace/blueprint.json`, set `status.publish` to `"complete"` and add `"repo_url": "<url from Step 2>"`, then write it back.
>
> Report: "Published to <repo-url>"

**Wait for the Publish Agent to complete.**

---

## Phase 6 — Done

Once the Publish Agent completes, print:
...
╚══════════════════════════════════════════════════════╝
  ...
  Agents run  : 8  (Architect, Frontend, Backend,
                    Database, DevOps, Tests, Reviewer,
                    Publish)
  Output      : ./workspace/
  GitHub repo : {blueprint.repo_url}
